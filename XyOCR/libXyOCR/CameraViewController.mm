       ...
       fromConnection:(AVCaptureConnection *)connection
{
    ...
    if (_nTimeCount < 0)
    {
        ...
        //发送通知
        [[NSNotificationCenter defaultCenter] postNotificationName:NotificationGetIDInfo object:@"NO"];
...
//                NSLog(@"_pixelLensCount = %d" , _pixelLensCount);
                //连续两次镜头位置不变，对焦成功
                _pixelLensCount--;
                if ([self.cardRecog newLoadImageWithBuffer:baseAddress Width:width Height:height] == 0) {
                    ...
                    //找边成功，1033，1034，1036代表机读码三种类型
                    if (sliderLine.allLine == 1 || sliderLine.allLine == 1033 || sliderLine.allLine == 1034 || sliderLine.allLine == 1036)
                    {
...
-(void)readyToRecog
{
    ...
    if (![allResult isEqualToString:@""]) {
        ...
        if (self.bIfHaveCheckPage)
        {
            ...
            checkViewController.pageType = 0;   //确认什么的号码 0 身份证  1 卡号
            checkViewController.bIfCheckViewNoEdit = self.bIfCheckViewNoEdit;
            checkViewController.bIfShowUserName = self.bIfShowUserName;
        ...
        else
        {
            ...
            {
                NSLog(@"CameraViewController 发送通知...通知跳自定义页面");
...
- (void)returnAndRemoveViewBG
{
    //代理对象事项协议返回
    if ([self.delegate respondsToSelector:@selector(removeViewBG)]) {
        [self.delegate removeViewBG];
    }
}
